...
使用 `mk.ViewBase` 或 `mk.StaticViewBase` 代替 `Component`
...
`mk.ViewBase` 和 `mk.StaticViewBase` 都是继承于 `mk.LifeCycle`，有相同的生命周期回调函数。
...
| 函数名   | 何时被调用                                  | 描述                            |
| -------- | ------------------------------------------- | ------------------------------- |
| `create` | 动态视图：添加父节点后。静态视图：onLoad 时 | onLoad 的替代品，用于初始化视图 |
| `open`   | create -> 递归子组件 create 和 open-> open  | 模块打开回调                    |
| `close`  | close -> 递归子组件 close                   | 模块关闭回调                    |

...
本文提供工程可直接复用的 MVVM 规范与模板。设计原则简明：
...
-   `monitor` 与 `ToolMonitorDataMethod` 自动完成数据到 UI 的同步，显著减少样板代码。
...
-   **View**
    ...
    -   使用 `monitor.on` / `ToolMonitorDataMethod` 将 `data` 与节点绑定（View 自己监听自己的 `data`）。
...
```ts
class MyModel extends MVCModelBase {
    items: string[] = [];
    gold = 1000;
...
### 2) View（视图层 — 持有 view-only `data` 并监听；对外提供更新接口）

```ts
class MyView extends MVCViewBase<MyModel> {
...
### 3) Control（ViewModel / 命令中心 — 仅调用 View 的事件与公开函数）

```ts
class MyControl extends MVCControlBase<MyModel, MyView> {
...
-   `MVCControlBase.close()` 的行为：等待 open 完成 → `model.close()` → `uiManage.close(view)` → `monitor.clear(this)`（清理 Control 注册的监视器）。
-   `MVCModelBase.close()` 会 `monitor.clear(this)` 并根据 `_isResetData` 重置数据。
-   注册 `monitor.on` 时将 `target` 设为 `this`（Control 或 View），以便框架在 close 时自动清理。

示例（在 View 中）：

```ts
monitor.on(this.data, 'label', handler, this); // target=this，框架会自动 clear
```
...
if ( 关 闭 ) then (是)
  :Control.close();
  :Model.close() -> 清理监视器;
  :uiManage.close(view) -> View.close() -> 清理视图监视器;
  stop
else (否)
  :继续运行;
  stop
...
-   `monitor` 与 `ToolMonitorDataMethod` 将数据驱动 UI 的优势最大化，建议把 Model → View 的转换逻辑集中在 Control（构建 snapshot）并交由 View 执行最终的 `data` 更新。

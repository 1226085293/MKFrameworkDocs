...
-   source 可以传递 Record 对象用于代表不同 UI 类型的资源来源
-   会在内部为每个不同 `type` 的资源建立资源对象池
...
await mk.uiManage.regis(
    Test,
    {
        ...
        default: 'Module/Test/Test222.prefab',
    },
    this
);
```
...
-   `config.type`：模块类型
-   `config.isAll`：关闭全部模块
-   `config.isDestroy`：销毁节点
-   `config.isDestroyChildren`：销毁动态子节点（动态子节点是通过 `mk.uiManage.open` 加载到模块节点下的其他模块）

...
### 资源跟随释放对象
    ...
    什么是 **资源跟随释放对象**？ 资源跟随释放对象的类型是 `Node | TypeFollowReleaseObject | null`，

    - 传递 null 时：
        代表自行管理资源，你只需要手动调用资源的 `decRef` 后资源即会被释放（加载几次则调用几次）

    - 传递 Node 时：
        框架内的 `MKFollowNodeRelease` 组件将在节点销毁后调用一次资源的 `decRef`

    - 传递 TypeFollowReleaseObject 对象时：

        `TypeFollowReleaseObject` 结构如下
        ![跟随释放对象类型](/docs/f9dfeba8-7279-4679-848b-baceabf87b20.png)

        followRelease: `mk.asset` 会在资源加载完成时调用跟随释放对象的 `followRelease`，参数为资源，然后由跟随释放对象自己决定什么时候调用资源的 `decRef`
...
-   `pathStr_`: 资源路径
    ...
    -   **编辑器环境**
        ...
        受环境限制，编辑器加载图片只支持 `ImageAsset` 类型
...
-   `config_.progressFunc`: 加载进度回调
-   `config_.completedFunc`: 加载完成回调
...
-   GlobalConfig.Asset.config.cacheLifetimeMsNum: 缓存生命时长（毫秒，资源未使用时经过多久释放）

-   GlobalConfig.Asset.config.retryCountOnLoadFailureNum: 加载失败重试次数

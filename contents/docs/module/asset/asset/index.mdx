...
A: 检查是否正确传入了 `target_`（跟随释放对象）。资源没被引用时会在缓存时间后才释放，配置在 `GlobalConfig.Asset.config.cacheLifetimeMsNum`

**Q：远程资源需要手动释放吗？**

A: 如果你在加载时传递了 `target_`（跟随释放对象），资源会由 `target_` 控制何时释放。
...
A: 可以，但会绕过引用计数与跟随释放机制，使用不当容易造成内存泄漏。推荐始终用 `mk.asset` 提供的封装接口。

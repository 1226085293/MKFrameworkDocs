...
[*] --> 引擎初始化 : 等待 Game.EVENT_GAME_INITED

state "启动流程" as Start {
  引擎初始化 --> 等待首场景: game inited
  等待首场景 --> 初始化 main bundle : Director.EVENT_BEFORE_SCENE_LAUNCH
  初始化 main bundle --> main.open : 调用 manage.init / open
  main.open --> 运行中
}

运行中 --> 请求 load(bundle) : 调用 mk.bundle.load
请求 load(bundle) --> 成功/返回 bundle
成功/返回 bundle --> 预加载场景?

alt 预加载
  --> bundle.preloadScene --> 预加载完成 --> 等待是否要真正切换
else 真正切换
  --> event.beforeBundleSwitch?
  --> event.beforeSceneSwitch?
  --> bundle.loadScene -> director.runScene
  --> 更新 bundleStr, sceneStr
  --> event.afterSceneSwitch / afterBundleSwitch
end

运行中 --> reload(bundle) : 热更（reload）
reload(bundle) --> 清理脚本缓存 / 注销 ccclass / 释放 bundle 资源
清理完成 --> load(new bundle)
...
participant "mk.bundle (MKBundle)" as MKBundle
participant "AssetManager\n(assetManager)" as AM
participant "BundleManage\n(manage)" as Manage
participant "Director" as Director

Dev -> User : 调用 mk.bundle.loadScene(scene, config)
User -> MKBundle : loadScene(scene, config)
activate MKBundle
MKBundle -> MKBundle : await init task
MKBundle -> MKBundle : load(bundle) 或 preloadScene
alt load bundle
  MKBundle -> AM : loadBundle(origin, {onFileProgress}, cb)
  AM --> MKBundle : bundle
end
MKBundle -> MKBundle : event.request(beforeBundleSwitch) // 可阻塞
MKBundle -> MKBundle : event.request(beforeSceneSwitch)
MKBundle -> AM : bundle.loadScene(scene, cb)
AM --> MKBundle : sceneAsset
MKBundle -> Director : runScene(sceneAsset, beforeLoad, launchedCb)
Director --> MKBundle : launchedCb(error, scene)
MKBundle -> MKBundle : 更新 bundleStr / sceneStr
MKBundle -> MKBundle : event.emit(afterSceneSwitch/afterBundleSwitch)
deactivate MKBundle

== reload ==
User -> MKBundle : reload(info)
MKBundle -> MKBundle : 清理脚本缓存、注销 ccclass、releaseAll、removeBundle
MKBundle -> MKBundle : load(new bundle)
```

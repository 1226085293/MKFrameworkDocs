...
我们建议采用 **继承机制** 来组织引导步骤，以便更好地管理和复用代码。

#### 创建引导步骤基类

首先，创建一个通用的引导步骤基类，继承自 `mk.GuideStepBase<typeof XXXGuideOperate.tab>`，其中 `XXXGuideOperate.tab` 是你在 [操作表](../guide#概念必读) 中定义的操作单元。
...
> **提示**：`XXX` 可替换为你的引导系统名称，例如 `Lobby`、`Game` 等，以便更好地组织模块。

---

## 注册步骤

虽然你已经创建了引导步骤脚本，但还需要将其注册到管理器中才能生效。

由于 `mk.GuideStepBase` 继承自 `Component`，你可以选择以下两种方式之一来实例化：

-   **挂载到节点上**
-   **直接使用 `new` 实例化**

无论哪种方式，最终都需要调用引导管理器的 `regis` 方法完成注册：

```ts
const stepInstance = new XXXGuideStepA();
guideManage.regis(stepInstance);
```

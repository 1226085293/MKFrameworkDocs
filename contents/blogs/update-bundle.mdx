...
其中 `index.dead1.js` 中的 `dead1` 就是 Decorator Bundle 的版本号

# 热更篇

首先你需要一个远程文件服务器，例如阿里云的 OSS，本地测试也可以使用 `npx http-server`
在当前目录开启一个 http 服务器
...
然后在游戏内通过 http 请求拉取远程版本文件可以获取最新的版本号，你可以根据业务逻辑自行添加更多数据，
例如更新失败后回退的版本号，不同平台的版本号等等..

### 检测更新

在首次加载 Bundle 前，需要调用 `mk.bundle.getCache`，根据返回值做不同处理

-   返回 null：代表没有加载过远程 bundle，可以通过 `cc.assetManager.downloader.bundleVers["bundle名"]`
    获取本地的 Bundle 版本和远程版本进行对比，如果不一致则代表可以进行热更

-   返回 Bundle 信息：返回值就是缓存中上次加载的远程 bundle 信息，使用 `versionStr` 和远程版本对比，
    不一致则代表存在更新，如果一致就使用 `getCache` 返回的信息加载 bundle（引擎已缓存不会重新下载）

// app/blog/page.tsx
import { Avatar, AvatarFallback, AvatarImage } from '@/components/ui/avatar';
import { formatDate, stringToTimeStamp } from '@/lib/utils';
import { Metadata } from 'next';
import Image from 'next/image';
import Link from 'next/link';
import { allBlogs } from 'contentlayer/generated';

export const metadata: Metadata = {
    title: 'MKFrameworkDocs - Blog',
};

// 定义作者类型
interface Author {
    avatar?: string;
    handle: string;
    username: string;
    handleUrl: string;
}

// 定义博客前言类型
interface BlogFrontmatter {
    title: string;
    description?: string;
    date: string;
    authors: Author[];
    cover: string;
}

// 获取所有博客的前言数据
function getAllBlogsFrontmatter() {
    return allBlogs
        .map((blog) => ({
            ...blog,
            slug: blog.slug,
            date: blog.date,
            title: blog.title,
            description: blog.description || '',
            cover: blog.cover,
            authors: blog.authors || [],
        }))
        .sort((a, b) => stringToTimeStamp(b.date) - stringToTimeStamp(a.date));
}

export default function BlogIndexPage() {
    const blogs = getAllBlogsFrontmatter();

    return (
        <div className="flex flex-col gap-1 sm:min-h-[91vh] min-h-[88vh] pt-2">
            <div className="mb-7 flex flex-col gap-2">
                <h1 className="sm:text-3xl text-2xl font-extrabold">最近博客</h1>
                <p className="text-muted-foreground sm:text-[16.5px] text-[14.5px]">
                    关于本产品的博客及公告
                </p>
            </div>
            <div className="grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1 sm:gap-8 gap-4 mb-5">
                {blogs.map((blog) => (
                    <BlogCard
                        key={blog.slug}
                        date={blog.date}
                        title={blog.title}
                        description={blog.description}
                        slug={blog.slug}
                        cover={blog.cover}
                        authors={blog.authors}
                    />
                ))}
            </div>
        </div>
    );
}

interface BlogCardProps extends BlogFrontmatter {
    slug: string;
}

function BlogCard({ date, title, description, slug, cover, authors }: BlogCardProps) {
    return (
        <Link
            href={`/blog/${slug}`}
            className="flex flex-col gap-2 items-start border rounded-md py-5 px-3 min-h-[400px] min-w-[360px]"
        >
            <h3 className="text-md font-semibold -mt-1 pr-7">{title}</h3>
            <div className="w-full">
                <Image
                    src={cover}
                    alt={title}
                    width={400}
                    height={150}
                    quality={80}
                    className="w-full rounded-md object-cover h-[180px] border"
                />
            </div>
            <p className="text-sm text-muted-foreground">{description}</p>
            <div className="flex items-center justify-between w-full mt-auto">
                <p className="text-[13px] text-muted-foreground">发布于 {formatDate(date)}</p>
                <AvatarGroup users={authors} />
            </div>
        </Link>
    );
}

function AvatarGroup({ users, max = 4 }: { users: Author[]; max?: number }) {
    const displayUsers = users.slice(0, max);
    const remainingUsers = Math.max(users.length - max, 0);

    return (
        <div className="flex items-center">
            {displayUsers.map((user, index) => (
                <Avatar
                    key={user.username}
                    className={`inline-block border-2 w-9 h-9 border-background ${
                        index !== 0 ? '-ml-3' : ''
                    } `}
                >
                    <AvatarImage src={user.avatar} alt={user.username} />
                    <AvatarFallback>{user.username.slice(0, 2).toUpperCase()}</AvatarFallback>
                </Avatar>
            ))}
            {remainingUsers > 0 && (
                <Avatar className="-ml-3 inline-block border-2 border-background hover:translate-y-1 transition-transform">
                    <AvatarFallback>+{remainingUsers}</AvatarFallback>
                </Avatar>
            )}
        </div>
    );
}
